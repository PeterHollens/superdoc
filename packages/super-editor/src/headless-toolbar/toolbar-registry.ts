...
export const createToolbarRegistry = (): Partial<Record<PublicToolbarItemId, BuiltInToolbarRegistryEntry>> => {
  return {
    ...
    link: {
      ...
      // Follow-up work: item-specific execute should absorb link apply/remove semantics
      // from the legacy toolbar flow (including post-submit cursor/focus recovery).
